...
def run_episode(
    ...
    final_transition = env._last_transition
    assert final_transition is not None
    after = final_transition.after
...
class DiagnosisProbeCallback(BaseCallback):
    ...
    def _run_probe(self) -> None:
        if not self._oracle_written:
            ...
            self._write(
                "diagnosis_probe_oracle.txt",
                report(traces, "ground oracle (reference)"),
            )
        ...
        self._write(
            ...
            report(traces, f"checkpoint@{self.num_timesteps}"),
        )
        if self.verbose:
            print(
                f"[diagnosis] wrote probe report at "
